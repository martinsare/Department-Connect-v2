...
import type {
  ClassSession,
  ClassAttendee,
} from "@/frontend/context/DataContext";
import { Avatar } from "@/frontend/components/Avatar";
import { QR_ATTENDANCE_TOKEN as QR_TOKEN } from "@/frontend/data/seedData";
...
function AttendeeRow({ item, index }: { item: ClassAttendee; index: number }) {
  ...
  const studentPic = students.find(
    (s) => s.matricNumber === item.matricNumber
  )?.profilePicture;
  ...
  return (
    <View style={[rosterStyles.row, { borderBottomColor: colors.border }]}>
      ...
      />
      <View style={{ flex: 1 }}>
        <Text style={[rosterStyles.name, { color: colors.foreground }]}>
          {item.name}
        </Text>
      ...
      <View
        style={[
          rosterStyles.timeBadge,
          { backgroundColor: colors.primary + "15" },
        ]}
      >
        <Ionicons name="time-outline" size={11} color={colors.primary} />
        <Text style={[rosterStyles.timeText, { color: colors.primary }]}>
          {item.scanTime}
        </Text>
      </View>
    </View>
...
export default function AdminClassesScreen() {
  ...
  const qrValue = qrClass
    ? JSON.stringify({
        classId: qrClass.id,
        courseCode: qrClass.courseCode,
        courseName: qrClass.courseName,
        token: QR_TOKEN,
      })
    : "{}";

  const rosterList: ClassAttendee[] = rosterClass
    ? classAttendees[rosterClass.id] ?? []
    : [];
        ...
        <View style={[styles.cardActions, { borderTopColor: colors.border }]}>
          <TouchableOpacity
            style={[
              styles.toggleBtn,
              {
                backgroundColor: cls.attendanceOpen
                  ? "#10B98115"
                  : colors.muted,
              },
            ]}
          ...
          >
            <Ionicons
              name={
                cls.attendanceOpen ? "lock-open-outline" : "lock-closed-outline"
              }
              size={14}
          ...
          >
            ...
            <Text style={[styles.rosterBtnText, { color: colors.primary }]}>
              Roster
            </Text>
          </TouchableOpacity>
    ...
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      ...
      >
        ...
        <Text style={styles.subtitle}>
          Manage attendance and view class rosters
        </Text>
      </LinearGradient>
      ...
      <Modal
        ...
        onRequestClose={() => setQrClass(null)}
      >
              ...
              <>
                ...
                <View
                  style={[
                    styles.openBadge,
                    {
                      backgroundColor: qrClass.attendanceOpen
                        ? "#10B98118"
                        : "#EF444418",
                    },
                  ]}
                >
                  ...
                  >
                    {qrClass.attendanceOpen
                      ? "Attendance is OPEN — students can scan"
                ...
                <Text
                  style={[styles.qrHint, { color: colors.mutedForeground }]}
                >
                ...
                <View style={styles.modalActions}>
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      handleToggle(qrClass);
                      setQrClass((prev) =>
                        prev
                          ? { ...prev, attendanceOpen: !prev.attendanceOpen }
                          : null
                      );
                    }}
                    activeOpacity={0.8}
      ...
      <Modal
        ...
        animationType="slide"
        statusBarTranslucent
        onRequestClose={() => setRosterClass(null)}
      >
        <View style={styles.modalBackdrop}>
            ...
            <View style={styles.rosterFixed}>
              ...
              <View style={styles.rosterHeader}>
                <View style={{ flex: 1 }}>
                  ...
                  {rosterClass && (
                    <Text
                      style={[styles.modalCourse, { color: colors.primary }]}
                    >
              ...
              {rosterClass && (
                ...
                >
                  <View style={styles.rosterStat}>
                    <Text
                      style={[styles.rosterStatVal, { color: colors.primary }]}
                    >
                      {rosterList.length}
                    </Text>
                    <Text
                      style={[
                        styles.rosterStatLabel,
                        { color: colors.mutedForeground },
                      ]}
                    >
                      Present
              ...
              {rosterClass && (
                <TouchableOpacity
                  style={[
                    styles.rosterToggle,
                    {
                      backgroundColor: rosterClass.attendanceOpen
                        ? "#10B98115"
                        : "#EF444415",
                    },
                  ]}
                ...
                >
                  ...
                  <Text
                    style={[
                      styles.rosterToggleText,
                      {
                        color: rosterClass.attendanceOpen
                          ? "#10B981"
                          : "#EF4444",
                      },
                    ]}
