...
import type { AdminNotification } from "@/frontend/context/DataContext";
...
export default function AdminNotificationsScreen() {
  ...
  const derived: AdminNotif[] = [
      ...
      ? [
          {
            ...
            body: `${pendingPayments
              .map((p) => p.submittedBy?.name ?? "Student")
              .slice(0, 3)
              .join(", ")}${
              pendingPayments.length > 3
                ? ` +${pendingPayments.length - 3} more`
                : ""
            } claimed transfer. Go to Payments to confirm.`,
            time: "Now",
            isRead: false,
      ...
      ? [
          {
            ...
            iconColor: "#F59E0B",
            title: `${pending.length} Pending Approval${
              pending.length !== 1 ? "s" : ""
            }`,
            body: `${pending
            ...
            } are awaiting document verification.`,
            time: "Today",
            isRead: false,
      ...
      ? [
          {
            ...
            body: todayClasses
              .map((c) => `${c.courseCode} at ${c.startTime}`)
              .join(" · "),
            time: "Today",
            isRead: false,
            priority: "normal" as const,
      ...
      ? [
          {
            ...
            iconColor: "#EF4444",
            title: `${unpaidContributions.length} Unpaid Contribution${
              unpaidContributions.length !== 1 ? "s" : ""
            }`,
            body: `₦${unpaidContributions
              .reduce((s, c) => s + c.amount, 0)
              .toLocaleString()} outstanding across ${
              unpaidContributions.length
            } student${unpaidContributions.length !== 1 ? "s" : ""}.`,
            time: "This week",
            isRead: true,
            priority: "normal" as const,
          },
    ...
    ...announcements.slice(0, 2).map((a, i) => ({
      ...
      body: `Posted to ${a.targetAudience}${
        a.postedBy ? ` by ${a.postedBy}` : ""
      }.`,
      time: a.time,
      isRead: true,
      priority: "normal" as const,
